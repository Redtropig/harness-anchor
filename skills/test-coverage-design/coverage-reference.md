...
| **Shared mutable state across threads** | data race, torn read, lost update; **check-then-act on a composite predicate** (e.g. waiting on `count==0 && inflight==0` across a non-atomic window → premature wake) | concurrent producer / consumer under stress, many iterations; for the composite-predicate case assert the waited-on condition actually held at wake (no TOCTOU window) |
...
| **Behavioural-contract substitution** (swap a container / algorithm / impl behind a stable API or signature) | a previously-guaranteed *observable* property silently regresses — ordering / FIFO / stability / insertion-order tiebreak, idempotency, a documented no-op-on-X, or a public signature / return type the old call sites relied on | **characterization** (record the OLD behaviour as the oracle *before* the swap, assert the new impl still matches) or **metamorphic** (assert the invariant directly: equal-key inputs keep insertion order; re-applying the op changes nothing; the op on an already-satisfied precondition is a no-op) |
| **Liveness / termination under adversarial structure** (graphs, dependencies, retries, recursion) | hang / deadlock / infinite loop on a cycle, self-edge, or already-satisfied precondition — the happy path terminates, so the suite never sees it | feed the adversarial input (a cycle `A↔B`, a self-edge, an already-complete upstream) and assert **bounded termination or an explicit diagnostic** (a deadline / error-return oracle), never a hang; prefer an API that *diagnoses* (returns an error) over one that only relies on a timeout |
