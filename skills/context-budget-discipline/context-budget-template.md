...
| harness-anchor injection (= the three parts below) | this plugin SessionStart | 1 | **≤2000 t (8000-char hard cap); ~1900 measured at v0.9.0** | — | treat as full |
...
| ├ TOC view | `<project-toc>` adaptive block | 1 | leftover-sized (~250 t on a small repo) | — | full `## Files` → `## Directory map` → top-level dirs, as budget allows |
| └ meta-skill body | using-harness-anchor SKILL.md | 1 | ~1500 t | — | truncated last if the cap would be exceeded |
